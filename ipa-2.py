    ...
    if letter==" ":
        ...
        for i in letter:
            if shift=="0":
                ...
                new_str += chr(((ord(i)-65+shift)%26)+65)
            else:
                new_str += "error"
    return (new_str)
...
def scytale_cipher(message, shift):
    ...
    if len(message)% shift == 0:
        ...
        new_str = message + ("_" * (shift - len(message)% shift))
    
    
    cipher = ""
    
    for i in range(len(new_str)):
        cipher += new_str[(i // shift) + (len(new_str) // shift) * (i % shift)] 
...
def scytale_decipher(message, shift):
    ...
    for i in range(len(message)):
        decipher += message[((i // (len(message)//shift)) + (len(message)//(len(message)//shift)) * (i % (len(message)//shift)))]
